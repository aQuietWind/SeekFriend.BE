...
public class JWTUtil {
    ...
    //生成令牌的方法
    public static String obtainJwt(long tokenId,String secretKey,long expireTime){
        ...
        return Jwts.builder()
                ...
                .subject("seek_friend")          //设置主题
                .issuedAt(new Date())               //设置签发时间
                .expiration(usefulTime)             //设置最终有效期
                .signWith(key)                      //设置该令牌密钥，自动识别HS256算法
                .compact();                         //根据上述设置生成一个令牌字符串
    }

    //解析并获取令牌内容的方法
    public static Long jwtCheck(String token,String secretKey){
        SecretKey key= Keys.hmacShaKeyFor( secretKey.getBytes(StandardCharsets.UTF_8) );    //根据字符串密钥生成HS256形式的密钥
    ...
    public static TokenCheckResult jwtCheckByList(String token,String headerTokenSeparator,HashMap<String,String> jwtHeaders){
        //分割token,并且赋值
        String[] body=token.split(headerTokenSeparator,2);
